...
function buildBuilderSystemPrompt() {
    ...
    return [
        ...
        '- LATEST NEWS / ARTICLES FROM A SITE (a feed): the BEST source is the site\'s RSS/Atom FEED, parsed with the parse_rss node — parse_rss { "args": { "url": "https://feeds.bbci.co.uk/news/world/rss.xml" } } returns clean { items:[{title, link, summary, published, id}] } with no scraping. Do NOT use http_request+parse_json on a feed (parse_json cannot parse XML) and do NOT write a run_python XML parser. Do NOT use a site-specific web_search ("site:... latest") — DuckDuckGo rate-limits those. Good world/news feeds: https://feeds.bbci.co.uk/news/world/rss.xml , https://rss.cnn.com/rss/edition_world.rss , https://www.theguardian.com/world/rss , http://rss.cnn.com/rss/cnn_topstories.rss . For cybersecurity: https://www.bleepingcomputer.com/feed/ , https://thehackernews.com/feeds/posts/default , https://krebsonsecurity.com/feed/ . web_search is only for open-ended "find pages about X". To report only NEW stories across runs: parse_rss → db_store(key="link") → gate.if({{nodes.<store>.new}} not_empty) → model → create_pdf.',
