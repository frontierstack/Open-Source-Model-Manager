...
- Monolithic Express application in `webapp/server.js` (~424KB file)
...
- Each instance gets unique port allocation (starting from 8001+)
- Containers bind to `127.0.0.1` only (not exposed to network)
...
- `api-keys.json` - Generated API credentials (AES-256-GCM encrypted)
- `conversations/` - Chat conversation history (per-user subdirectories)
  - `index.json` stores metadata + `messageCount`; message files are `{conversationId}.json`
  - `messageCount` backfilled on first list if missing from legacy conversations
- `.session-secret` - Persisted session secret (survives restarts)
- `.encryption-key` - API key encryption key (mode 0600)
...
**Skill Execution:**
1. Python code written to `/tmp/skill_<timestamp>_<crypto-random>.py`
2. Parameters passed via JSON file with crypto-random naming (avoids shell escaping)
3. Executed with 30s timeout, 10MB buffer limit
4. JSON output parsed and returned
5. Built-in skills have SSRF protection (`isPrivateUrl()` blocks private IPs)
6. Shell metacharacters stripped from command arguments (git, netstat skills)
...
- Extracts keywords from user's query (filters stop words, extracts phrases)
- Scores sentences by relevance to query keywords
- Bonus scoring for sentences with numbers, "must/required/critical" keywords
- Keeps most relevant sentences (default: 40% of content)
- May avoid chunking entirely if condensed content fits context window

Key functions in `webapp/server.js`:
- `extractQueryKeywords(query)` - Extract meaningful keywords from user query
- `scoreSentenceRelevance(sentence, keywords)` - Score sentence relevance
- `condenseContent(content, query, targetRatio)` - Query-focused extraction
- `splitIntoChunks(content, chunkSize, overlap)` - Split with overlap
- `processWithMapReduce(options)` - Orchestrate parallel chunk processing
...
- `webSearchEnabled` and `urlFetchEnabled` always default to `false` on app start (not persisted)

**Status Indicator Types** (`chat/src/components/chat/StatusIndicator.jsx`):
- `IDLE` - No active processing
- `THINKING` - Model is processing (purple, spinning)
- `SEARCHING` - Web search in progress (blue, pulsing)
- `PARSING` - Parsing files/attachments (amber, pulsing)
- `PROCESSING` - General processing (emerald, pulsing)
- `GENERATING` - Streaming response (primary color, pulsing)
- `CHUNKING` - Processing content chunks (cyan, pulsing)
- `SYNTHESIZING` - Combining chunk responses (violet, pulsing)
...
- Font family selection (50+ fonts: Sans-serif, Serif, Monospace categories)
  - Font CSS uses high-specificity `html body.font-family-*` selectors outside `@layer` to override Tailwind
- Theme selection (23+ themes including nature, dev, warm tones, neutral; Midnight and Synthwave removed)
...
**Authentication:** Sessionless connections allowed (for startup race conditions) but won't receive user-targeted messages. Session binding attempted via `modelserver.sid` cookie parsing.

**Auto-Reconnection (webapp App.js):** Exponential backoff (1s→10s, max 20 attempts).
...
- `POST /api/chat/stream` - Streaming chat completions (SSE)
  - Supports `chunkingStrategy`: `'auto'` | `'map-reduce'` | `'truncate'` | `'none'`
  - Auto-detects large content and applies map-reduce with content condensation
  - Returns chunking progress events and synthesis status
...
- `GET /api/chat/continuation/:conversationId` - Check chunked content queue status
- `DELETE /api/chat/continuation/:conversationId` - Clear chunked content queue
...
- Continue button for length-limited responses (when `finish_reason: 'length'`)
- Manual stop saves partial content normally (no false "cut off" error)
- Continue sends prompt to API only (not visible in chat as user message)

**Web Search (Chat Integration):**
- Triggered when `webSearchEnabled` is on (off by default each session)
- Fetches 5 results with full page content (up to 4000 chars each)
- Search prompt instructs model to use specific details (CVEs, numbers, dates) and cite sources
- Search results prepended to user message with explicit extraction instructions
...
port = findNextAvailablePort(); // Starting from 8001
...
**Entry point:** `agents-cli/bin/koda.js` (~329KB monolithic file)
...
**Implemented:**
- Helmet HTTP security headers (CSP without unsafe-eval, X-Frame-Options, same-origin resource policy)
- bcryptjs password hashing (cost 10)
- Rate limiting on auth (15min/10 attempts), API (60s/100 requests), CLI install, and has-users endpoints
- Permission-based access control with field-whitelisted user updates
- HttpOnly, Secure, SameSite=Lax cookies
- Timing-safe API key/secret comparison (`crypto.timingSafeEqual`)
- SSRF protection (`isPrivateUrl()`) on fetch_url and http_request skills
- Command injection prevention (shell metachar stripping in git/netstat skills)
- ReDoS protection on user-supplied regex patterns
- API secrets masked in responses (last 4 chars only)
- Symlink detection in file path validation (`fs.realpathSync`)
- Model config input validation (parameter range checking)
- Session secret persisted to file (survives container restarts)
- Conversation ID format validation (prevents path traversal)
- Model containers bound to 127.0.0.1 (not network-exposed)
- JSON body limit: 10MB; file upload limit: 10MB
- Password change restricted to session auth only
- Generic login error messages (prevents user enumeration)
- Temp files use `crypto.randomBytes` naming
- Python dependencies pinned in Dockerfile

**Known Gaps:**
- No email verification
- No audit logging
- No CSRF tokens (partially mitigated by SameSite=Lax)
- Docker socket mounted (required for container management)
- Webapp container runs as root (required for Docker socket access)
- Python skill code not sandboxed beyond auth requirement
...
| `SESSION_SECRET` | auto-generated & persisted | Session encryption key (saved to `.modelserver/.session-secret`) |
...
**SSL/TLS Bypass (Corporate Proxies):**

Corporate networks often use SSL inspection (MITM proxies) that break certificate verification. The system handles this automatically:

1. `build.sh` auto-detects SSL inspection and creates `.env` with `NODE_TLS_REJECT_UNAUTHORIZED=0`
2. This enables bypass in Node.js, Python (requests, urllib3), and curl_cffi (Scrapling)
3. Manual override: `NODE_TLS_REJECT_UNAUTHORIZED=0 ./start.sh`
